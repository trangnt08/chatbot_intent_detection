...
def load_data(filename, dict):
    ...
    with open(filename, 'r') as f,open(dict, "w") as f2:
        ...
        if filename == 'data/ques2.txt':
            joblib.dump(train,'model/train2.pkl')
        else:
            joblib.dump(train,'model/test2.pkl')
    return train
...
def training1():
    ...
    # if vectorizer == None:
    vectorizer = TfidfVectorizer(ngram_range=(1, 1), max_df=0.7, min_df=2, max_features=1000)
    train = load_model('model/train2.pkl')
    if train == None:
...
def fit1(X_train,y_train):
    ...
    joblib.dump(uni_big, 'model/unigram.pkl')

def predict_ex(mes):
    print mes
    uni_big = load_model('model/unigram.pkl')
    if uni_big == None:
        training1()
    uni_big = load_model('model/unigram.pkl')
...
def test_file():
    ...
    vectorizer = load_model('model/vectorizer2.pkl')
    # iterate over classifiers
    test = load_model('model/test2.pkl')
    if test is None:
